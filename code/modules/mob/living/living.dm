	...
	if(ismob(AM))
		...
		if(!supress_message && !(iscarbon(AM) && HAS_TRAIT(src, TRAIT_STRONG_GRABBER)))
			if(ishuman(M))
				...
				var/grabbed_by_hands = grabbed_human.usable_hands > 0
				M.visible_message(span_notice("[src] grabs [M] [grabbed_by_hands ? "by their wrist":"passively"]..."), \
								span_warning("[src] grabs you [grabbed_by_hands ? "by your wrist":"passively"]."), null, null, src)
				to_chat(src, span_notice("You grab [M] [grabbed_by_hands ? "by their wrist":"passively"]."))
...
/mob/living/throw_at(atom/target, range, speed, mob/thrower, spin=0, diagonals_first = 0, datum/callback/callback, force, gentle = FALSE, quickstart = TRUE, throw_type_path = /datum/thrownthing)
	stop_pulling()
	. = ..()
