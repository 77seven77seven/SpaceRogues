...
/datum/species/proc/handle_radiation(mob/living/carbon/human/source, time_since_irradiated, seconds_per_tick)
	...
	if(time_since_irradiated > RAD_MOB_METAL && SPT_PROB(RAD_MOB_METAL_PROB, seconds_per_tick))
		to_chat(src, span_notice("You can taste metal."))
	...
	switch(adjusted_pressure)
		...
		// Very low pressure, show an alert and take damage
			...
			if(HAS_TRAIT(H, TRAIT_RESISTLOWPRESSURE))
				...
				H.adjust_brute_loss(pressure_damage, required_bodytype = BODYTYPE_ORGANIC)
				H.adjust_fire_loss(pressure_damage, required_bodytype = BODYTYPE_ORGANIC)
